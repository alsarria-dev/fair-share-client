    ...
    if (group.groupAuthor._id !== user._id) {
      ...
      if (editMode) {
        const inputs = {
          ...
          description,
          groupAuthor:
            selectAdmin === null ? group.groupAuthor._id : selectAdmin,
        };
        axios
          .put(`${API_URL}/groups/${groupId}`, inputs, {
            headers: { Authorization: `Bearer ${storedToken}` },
          })
          .then((response) => {
              ...
              <div className="titleAndBtns">
                ...
                <div className="div-left">
                  ...
                  {editMode ? (
                    ...
                    <p>
                      <span>Admin: </span>
                      {`${group.groupAuthor.name} ${group.groupAuthor.lastName}`}
                    </p>
                  )}
                  <p>
                    <span>Date: </span>
                ...
                <div className="detailsMetricsWrap">
                  ...
                  >{`Total Balance: ${group.groupUsers.filter((userSarch) => userSarch._id === user._id)[0].balance} €`}</p>
                </div>
                <div className="Btns">
                  <button
                  ...
                  <button onClick={handleEditMode} className="button-details">
                    {editMode ? "Save" : "Edit Group"}
                  </button>
                  {editMode && (
                    <button
                  ...
                  <button onClick={deleteGroup} className="button-details">
                    Delete Group
                  </button>
                  {editGroupMessage && (
                    <p className="red">{editGroupMessage}</p>
                  )}
                </div>
              </div>

              {group.groupExpenses ? (
                group.groupExpenses.map((expense) => {
                  return (
                    <ExpenseCard
                      key={expense._id}
                      expense={expense}
              ...
              return (
                <div key={user._id} className="profile-details">
                  ...
                  <div>
                    ...
                    <p>
                      {user.balance < 0 ? "Owes: " : "Receives "}
                      <span
                        className={user.balance < 0 ? "red" : "green"}
