...
function ExpenseCard({ expense, getGroup, groupId }) {
  ...
  const handleSubmit = () => {
    ...
    if (Object.values(formData).includes("")) {
      ...
      if (expenseInfo.expenseAuthor._id !== user._id) {
        ...
        axios
          .put(`${API_URL}/expenses/${expense._id}`, formData, {
  ...
  const displayInfo = () => {
    axios
      .get(`${API_URL}/expenses/details/${expense._id}`, {
        headers: { Authorization: `Bearer ${storedToken}` },
      })
      .then((response) => {
  ...
  const getPayers = () => {
    axios
      .get(`${API_URL}/groups/details/${groupId}`, {
        headers: { Authorization: `Bearer ${storedToken}` },
      })
      .then((response) => {
      ...
      {expenseInfo && (
        <div className={showInfo}>
          <div className="imgInDetails">
            <img
              className="trash-bin-image"
              src={editMode ? closeBtn : glassIcon}
              alt=""
              onClick={displayInfo}
            />
            <img
          ...
          <div className="expense-details-info">
            <form className="column">
              ...
              {editMode ? (
                <Select
                  options={selectPayer}
                  name="expenseAuthor"
                  onChange={(e) => handleChangeUsers(e, "author")}
                />
              ) : (
                <p>
                  <span>Paid by: </span>
                  {`${expenseInfo.expenseAuthor.name} ${expenseInfo.expenseAuthor.lastName}`}
                </p>
              )}
              <p>
                <span>Expense Date: </span>
                {`${expenseInfo.createdAt.split("T")[0]}`}
              </p>
              <p>
                <span>Last Update: </span>
                {`${expenseInfo.updatedAt.split("T")[0]}`}
              </p>
              {editMode ? (
                <input
                  type="number"
              ...
              {editMode ? (
                <Select
                  options={selectPayer}
                  name="expenseUsers"
                  onChange={(e) => handleChangeUsers(e, "payers")}
                  isMulti
                />
              ) : (
                <ul>
                  ...
                  {expenseInfo.expenseUsers.map((user, index) => {
                    return (
                      <li
                        key={`expense_card ${index}`}
                      >{`${user.name} ${user.lastName}: ${(expenseInfo.amount / expenseInfo.expenseUsers.length).toFixed(2)} €`}</li>
                    );
                  })}
                </ul>
              )}
            ...
            <div className="centered-div">
              {editMode ? (
                ...
                <img
                  className="expense-picture"
                  src={expenseInfo.expensePic}
                  alt=""
                />
              )}
              {editMode && (
                <img className="expense-picture" src={formData.expensePic} />
              )}
            </div>
            {editMode && (
              <button className="button-details-expense" onClick={handleSubmit}>
                Save
              </button>
            )}
            {editMode && (
              <button
                className="button-details-expense"
            ...
            {editMessage && (
              <p
                className={editMessage === "Expense updated" ? "green" : "red"}
              >
                {editMessage}
              </p>
            )}
