...
function App() {
    ...
    axios
      ...
      .then((response) => {
        ...
        const paid = Math.round(
          data.groupExpenses.reduce(
            (acc, curr) =>
              curr.expenseAuthor === user._id ? acc + curr.amount : acc + 0,
            0,
          ),
        );
        const borrowed = Math.round(
          data.groupExpenses.reduce(
            (acc, curr) =>
              curr.expenseAuthor !== user._id
                ? acc + Math.round(curr.amount / curr.expenseUsers.length)
                : acc + 0,
            0,
          ),
        );
        const total = Math.round(
          data.groupExpenses.reduce((acc, curr) => acc + curr.amount, 0),
        );
  ...
  const getBalance = (data) => {
    const user_set = data.groupUsers.map((user) => {
      return { id: user._id, paid: 0, contributed: 0 };
    });
